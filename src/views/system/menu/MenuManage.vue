<template>
  ...
  <!-- <MenuOpera v-if="dialogFormVisible" :key="dialogKey" :formValue="form" :operateType="operateType"
    :parentMenuList="parentMenuList" @operateBack="onMenuOpera" /> -->

    <MenuOpera v-if="dialogFormVisible" :key="dialogKey" :formInfo="gerenalFormInfo()" :operateType="operateType"
    :parentMenuList="parentMenuList" @operateBack="onMenuOpera" />
</template>

<script setup>
import { DeleteMenu, GetMenuPageList, GetMenuListByUser, GetParentMenuList } from "@/api/menu";
...
let tableColumn=[
    ...
    ,{ prop: "menuType", label: "菜单类型", sort: false, filterValue: '', width: 120, scope: true,type:'int',options:[{value: 0,label: "目录",},{value: 1,label: "组件",},]}
...
const GetData = () => { 
  ...
  GetMenuPageList(postJson).then(res => {
    if (res.code == "000") {
